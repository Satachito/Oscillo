...
| Logic D0–D7 | 6–13 |
| SG OUT | 22（基板上1kΩ経由） |
| 信号生成器 | 16 サイン, 17 ホワイト, 18 ピンク, 19 ブラウン（ファームウェア1.10以降） |

空きGPIOは 0, 1, 14, 15, 20, 21, 28 です。28だけがADC（ADC2）に繋がっています。
23/24/29はPico 2自身が使い、Pico 2 Wでは25も無線モジュール側に行きます。

信号生成器は他のボードではGPIO0–3ですが、この基板はそこがレンジ制御なので16–19に
移してあります。PWMスライスもSG OUT（22）と競合しません。ファームウェア1.9では
この基板だけ「生成器なし」と申告していました。各ピンはPWMキャリアなので、電圧として
使うにはRC（1kΩ + 100nF）が要ります。**起動時に16–19はLow出力として駆動されます**
ので、基板側がこれらのピンに何か繋いでいないかメーカー回路図で確認してください。
