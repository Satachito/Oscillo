...
An ArLyzer — this protocol on an Arduino Nano R4, UNO R4 Minima or UNO R4 WiFi
(`ArLyzer/`) — cannot offer the vendor interface: the board's stock Arduino core
builds its own USB descriptors and compiles TinyUSB's vendor class out. It
carries the same frames, byte for byte, over its USB serial port instead
(Arduino's `2341:0074` on a Nano R4 and `2341:0069` on a Minima, or `2341:0374`
and `2341:0369` from their bootloaders; `2341:1002` on a WiFi, whose USB port is
an ESP32-S3 passing the bytes on over a UART). A serial port has no packets, so
there are no zero-length packets either; the header's length is the only
boundary. Open it at 230400 baud — the WiFi's UART runs at the rate the port is
opened at, and its sketch listens at that one; the others ignore the rate — and
assert DTR. 1200 baud is the rate an Arduino takes as the signal to drop into
its bootloader.

A UART can lose a byte. A header whose length no request has is answered
`badLength` at once rather than waited on, and a frame that stops arriving part
way is dropped after 50 ms of quiet, so a lost byte costs one request, not every
one after it.
...
| 8 | `u32` | board id — 0 bare Pico 2, 1 PiLyzer AFE rev A, 3 PL2407AFE, 4 ArLyzer on an Arduino Nano R4, 5 ArLyzer on an Arduino UNO R4 Minima, 6 ArLyzer on an Arduino UNO R4 WiFi (2 is retired) |
