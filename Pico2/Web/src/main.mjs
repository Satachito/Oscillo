...
// A plain text field, as on the Mac, rather than a number field with the
// browser's stepper, which also nudged the value on a scroll or an arrow key.
function voltsField(input, read, write, after) {
  ...
  // A text field reports a change only when it loses focus, not on Enter.
  input.addEventListener('keydown', event => { if (event.key === 'Enter') commit(); });
}
// Which channel cards are open. A card folds and unfolds whether or not the
// channel is enabled — reading a disabled channel's calibration, or setting it
// before switching it on, is the ordinary case — so this is its own state and
// not the Enabled checkbox's.
const channelOpen = [];
function channelControls() {
  ...
  for (let i = 0; i < caps().channels; i++) {
    ...
    caret.onclick = () => {
      ...
      channelOpen[i] = now; body.hidden = !now;
      caret.textContent = now ? '▾' : '▸'; caret.setAttribute('aria-expanded', now);
    };
    ...
    SCALE_STEPS.forEach(v => scale.add(option(v, `${fmt(v, 'V')}/div`)));
