...
export const makeSettings = () => ({ mode: 'scope', timebase: .001, record: 2048, source: 0, trigger: 1, slope: 0, level: 0, position: .15, hysteresis: .004, lpf: 0, channels: Array.from({ length: MAX_ANALOG_CHANNELS }, () => ({ enabled: true, range: 0, probe: 1, scale: 0, offset: 0, ac: false, bias: 0, measuredBias: null, gain: [1, 1] })), testEnabled: true, testFrequency: 1000, signalsEnabled: false, signalSineHz: 440, spectrumSpan: 0, averaging: 1, xyX: 0, xyY: 1, logicTrigger: 1, logicSlope: 0, logicPosition: .15, spectrumWindow: 'Hann', spectrumScale: 'dBV', spectrumAveraging: 4, spectrumHarmonics: 5, spectrumLog: true, spectrumPeaks: true, decoder: 'None', uartParity: 'None', spiClock: 5, spiData: 6, spiSelect: 7, spiUsesSelect: true, spiIdleHigh: false, spiSecondEdge: false, i2cClock: 0, i2cData: 1, logInterval: .5, logicRate: 1000000, logicRecord: 4096, logicSource: 0, logicEnabled: 255, uartLine: 4, uartBaud: 115200, xy: false });
...
export class Acquisition {
  ...
  configure(settings) {
    ...
    this.pending = this.pending.catch(() => {}).then(async () => {
      ...
      if (instrument.caps.flags & 32) await instrument.setSignals(snapshot.signalsEnabled, snapshot.signalSineHz);
    });
    return this.pending;
  }
  async loop(settings, single, token) {
    ...
    try {
      ...
      if (instrument.caps.flags & 32) await instrument.setSignals(settings.signalsEnabled, settings.signalSineHz);
      let prepared;
  ...
  async capture(instrument, settings, actual, token) {
    if (settings.mode === 'meter') {
      let values;
      if (instrument.demo) values = Array.from({ length: instrument.caps.channels }, (_, c) => tone(c, performance.now() / 1000) * (settings.channels[c].gain?.[settings.channels[c].range] ?? 1) * settings.channels[c].probe);
      else {
