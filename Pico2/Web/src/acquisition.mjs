...
// CH1 and CH2 arrive through a front end biased to mid rail on this bench,
// CH3 goes straight to the converter. A board that reports its own offset
// carries it in its range instead, so this is a starting point to type over.
export const DEFAULT_ZERO_VOLTS = [1.65, 1.65, 0];
export const makeSettings = () => ({ mode: 'scope', timebase: .001, record: 2048, source: 0, trigger: 1, slope: 0, level: 0, position: .15, hysteresis: .004, lpf: 0, channels: Array.from({ length: 3 }, (_, i) => ({ enabled: true, range: 0, probe: 1, scale: 0, offset: 0, ac: false, zero: Array(3).fill(DEFAULT_ZERO_VOLTS[i] ?? 0) })), testEnabled: true, testFrequency: 1000, logInterval: .5, logicRate: 1000000, logicRecord: 4096, logicSource: 0, logicEnabled: 255, uart: false, uartLine: 4, uartBaud: 115200, xy: false });
